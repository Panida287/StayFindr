...
/**
 * Represents a single room slide in the gallery.
 */
interface Room {
	id: number;
	imageUrl: string;
...
/**
 * Props for the RoomGallery component.
 */
...
/**
 * Responsive Swiper-based image carousel to showcase a collection of room types.
 * Used on the homepage or listing pages to let users browse and preview room styles.
 */
export const RoomGallery: React.FC<RoomGalleryProps> = ({ rooms }) => {
	return (
		<section
			className="bg-yellow-700 bg-opacity-20 py-12 px-4 md:px-10 lg:px-20 text-center"
			aria-label="Room selection gallery"
		>
			<div className="mb-8">
			...
			<button
				className="inline-flex items-center gap-2 bg-yellow-700 hover:bg-yellow-800 text-white rounded-full px-6 py-3 transition duration-300 shadow-md"
				aria-label="Explore all available suite types"
			>
				Explore All Suite
			</button>
		</section>
