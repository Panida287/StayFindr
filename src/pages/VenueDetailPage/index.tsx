...
import { Car, ChevronLeft, ChevronRight, Coffee, MapPin, PawPrint, Star, Wifi } from 'lucide-react';
...
export default function VenueDetailPage() {
	...
	const images = media?.length ? media : [{url: 'https://placehold.co/600x400', alt: 'No image'}];
		...
		<div className="p-6 max-w-5xl mx-auto">
			...
			<div className="space-y-4">
				...
				<div className="mt-4 grid grid-cols-2 sm:grid-cols-4 gap-3 text-sm text-gray-700">
					{meta?.wifi && <div className="flex items-center gap-2"><Wifi className="w-4 h-4" /> WiFi</div>}
					{meta?.parking &&
                        <div className="flex items-center gap-2"><Car className="w-4 h-4" /> Parking</div>}
					{meta?.breakfast &&
                        <div className="flex items-center gap-2"><Coffee className="w-4 h-4" /> Breakfast</div>}
					{meta?.pets &&
                        <div className="flex items-center gap-2"><PawPrint className="w-4 h-4" /> Pets Allowed</div>}
				</div>
				...
				<div className="mt-8 border-t pt-4">
					...
					<div className="flex items-center gap-4">
						<img
							src={venue.owner.avatar?.url || 'https://placehold.co/80x80'}
